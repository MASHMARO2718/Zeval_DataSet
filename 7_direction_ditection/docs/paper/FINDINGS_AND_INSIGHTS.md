...
### 評価指標
- **関節角度誤差（Joint Angle MAE）**: 3点がなす屈曲角の誤差（0°～180°）。関節のポーズ精度を表す。
- **方向角誤差（Direction Detection, θ・ψ）**: 腰基準の方向角の誤差（-180°～+180°）。θはXY平面、ψはXZ平面。体の向き・方向検出の精度を表す。
- **3D空間誤差**: ユークリッド距離（参考値として）
- **評価対象関節**: 12関節（肩、肘、手首、腰、膝、足首の左右）

論文では両方の角度指標を記載し、表・図のキャプションでどちらの指標かを明示する。
